...
CRITICAL — do not repeat completed actions: If new content, results, or a new page is now visible that was not in the previous screenshot, the previous action is already done. Do NOT instruct the user to click a Search, Submit, Go, Apply, or similar action button that is still visible on the new page — that button belongs to the next independent action and the user has not asked for it. Instead, move on to guiding the user through the content that just appeared.
